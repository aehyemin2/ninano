...
async function fetchPackedWind(
  ...
  if (cached) return cached;
  const wind = readWindF32(await fetchBuffer(url, signal), fieldSize);
...
async function fetchPackedFrame(
  ...
  const [scalarEntries, wind] = await Promise.all([
    ...
    fetchPackedWind(cacheNamespace, layerUrl("wind", fileName), fieldSize, signal),
  ]);
...
export async function loadSimulationFrame(
  ...
  if (dataset.source === "api" && dataset.layerApiBaseUrl) {
    ...
    return fetchPackedFrame(
      `api:${baseUrl}`,
      (layer) => `${baseUrl}/enso/layers/${layer}?${query}`,
      dataset.metadata,
      inputs,
      selectedVariable,
      signal,
  ...
  if (dataset.source === "packed" && dataset.packedBaseUrl) {
    ...
    return fetchPackedFrame(
      `static:${baseUrl}`,
      (layer, fileName) => `${baseUrl}/${layer}/${fileName}`,
      dataset.metadata,
      inputs,
      selectedVariable,
      signal,
